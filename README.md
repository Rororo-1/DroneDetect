...
## Схема:
Завантаження данних → кластеризація → навчання моделі виявлення і ідентифікації → детекція на ваших файлах
### Кластеризація
...
Також я використав preprocess_input для нормалізації замість ділення бітів на 255, адже VGG16 очікує BGR канали і віднімання середніх значень ImageNetq.
...
### Детекція
Щоб запустити готову модель і почати виявляти за допомогою неї, запустіть detect.py.
Потрібно ввести шлях до відео чи зображення де ви хочете виявити дрон, після чого модель почне працювати, потім ваш файл автоматично відкриється.
## Запуск
Для того аби почати роботу, необхідно спочатку запустити код для завантаження данних з keggle: download_dataset.py.
Після цього неоюхідно запустити clusterization.py для розбиття зображень на класи
Потім запускаємо навчання моделі у d_model_yolo.py і чекаєм закінчення навчання моделі
В фіналі запускаєм detect.py після чого необхідно вказати шлях до файлу по якому буде робитись детекція.
## Проблеми і покращення
Пртягом роботи, я зіткнувся з кількомапроблемами, що в основному стосувались завеликій важкості моделей.
Спочатку планувалось, що детекцією і ідентифікацією займатиметься база ResNet50 з 2-3 шарами голови перед останнім був би шар активації softmax для класифікації і останнім був би linear для детекції.
Однак, виявилось, що навчання займає забагато часу, до того ж програма час від часу крашилась, через брак оперативки.
В майбутньому, я планую допилити свою детекцію, а поки я заміню її yolo
Також, серед покращень, можна було б зробити графіки розподілу, відмалювати зображення і те, як вони розкидались по класам
Так як розбиття по класам відбувалось автоматично, потрібно вручну преедивитись і додати назви класам, замість 1,2,3...
